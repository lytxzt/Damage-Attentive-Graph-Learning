...
def draw_approximate_pic_2D(num_of_remain, positions_trajectory):
    trajectory_step = len(positions_trajectory)
    fig = plt.figure()
    # ax = Axes3D(fig)
    # fig.add_axes(ax)
    ...
    for i in range(3):
        ...
        for j in range(num_of_remain):
            if i > 0:
                ...
                # z = [positions_trajectory[i - 1][j, 2], positions_trajectory[i][j, 2]]
                # ax.plot(x, y, z, c='b')
                plt.quiver(x[0], y[0], x[1] - x[0], y[1] - y[0])

            plt.scatter(positions_trajectory[i][j, 0], positions_trajectory[i][j, 1], s=30, c=c)

    # ax.set_zlabel('Height', fontdict={'size': 15, 'color': 'black'})
    # plt.set_ylabel('Ground Y', fontdict={'size': 15, 'color': 'black'})
    # plt.set_xlabel('Ground X', fontdict={'size': 15, 'color': 'black'})
